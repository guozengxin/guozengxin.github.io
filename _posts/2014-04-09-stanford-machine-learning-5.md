...
假设所有的$K$都是一个有效的核并对应着的一特征映射函数$\phi$，对于有限的m个特征点，构造矩阵$ K_{ij}= K(x^{(i)}, x^{(j)}) $，这个矩阵称为**核矩阵**。那么如果$K$是一个有效的核，那么矩阵必须满足$$K_{ij}= K(x^{(i)}, x^{(j)})=\phi(x^{(i)})^T \phi(x^{(j)}) = \phi(x^{(j)})^T \phi(x^{(i)}) = K(x^{(j)}, x^{(i)}) = K_{ji}$$，因此矩阵$K$必须是对称的。
